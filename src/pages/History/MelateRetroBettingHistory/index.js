...
const MelateRetroBettingHistory = () => {
    ...
    // Format melate retro bet for display
    const formatMelateRetroBet = (bet) => {
    ...
    // Format melate retro result for display
    ...
    const fetchBettingHistory = async (page = 1, pageSize = 50) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/admin/melate-retro/betting-history`, {
                params,
                headers: {
            ...
            console.error('Error fetching melate retro betting history:', err);
        ...
        try {
            ...
            if (response.data && response.data.data) {
                ...
                const rows = data.map(item => [
                    item.planNumber || '',
                    item.username || '',
                    formatMelateRetroBet(item.bet),
                    item.betAmount || 0,
                    dayjs(item.createdAt).format('YYYY-MM-DD HH:mm:ss'),
                    formatMelateRetroResult(item.result),
                    item.winAmount || 0,
                ...
                link.setAttribute('download', `melate_retro_betting_history_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}.csv`);
    ...
    const historyColumns = [
        ...
        },
        {
            ...
            render: (result) => {
                if (!result) return '-';
                const numbers = Array.isArray(formatMelateRetroResult(result)) 
                    ? formatMelateRetroResult(result) 
                    : formatMelateRetroResult(result).split(' ');
                return (
    ...
    return (
        <div className="p-4">
            <Title level={2}>Melate Retro Betting History</Title>
            <Card>
...
export default MelateRetroBettingHistory;
