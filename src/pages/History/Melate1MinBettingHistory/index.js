...
const Melate1MinBettingHistory = () => {
    ...
    // Format melate 1min bet for display
    const formatMelate1MinBet = (bet) => {
    ...
    // Format melate 1min result for display
    ...
    const fetchBettingHistory = async (page = 1, pageSize = 50) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/admin/melate-1min/betting-history`, {
                params,
                headers: {
            ...
            console.error('Error fetching melate 1min betting history:', err);
        ...
        try {
            ...
            if (response.data && response.data.data) {
                ...
                const rows = data.map(item => {
                    let numbers = [];
                    const formatted = formatMelate1MinResult(item.result);
                ...
                link.setAttribute('download', `melate_1min_betting_history_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}.csv`);
    ...
    const historyColumns = [
        ...
        },
        {
            ...
            render: (result) => {
                if (!result) return '-';
                let numbers = [];
                const formatted = formatMelate1MinResult(result);
                if (Array.isArray(formatted)) {
                    numbers = formatted;
                } else if (typeof formatted === 'string') {
                    numbers = formatted.split(' ');
    ...
    return (
        <div className="p-4">
            <Title level={2}>Melate 1min Betting History</Title>
            <Card>
...
export default Melate1MinBettingHistory;
