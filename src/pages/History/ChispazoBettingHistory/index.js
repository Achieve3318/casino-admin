...
const ChispazoBettingHistory = () => {
    ...
    // Format chispazo bet for display
    const formatChispazoBet = (bet) => {
        ...
        // Process all bet entries
        bet.forEach((betData, idx) => {
            const betParts = [];
            
            // Draw type
            if (betData.drawType) {
                betParts.push(`[${betData.drawType === 'clasico' ? 'Clásico' : 'De Las Tres'}]`);
            }
            ...
            if (betData.numberSelection && betData.numberSelection.numbers) {
                ...
                const numberArrays = Array.isArray(numbers[0]) ? numbers : [numbers];
                numberArrays.forEach(numArray => {
                    betParts.push(`${gameTypeLabel}: [${numArray.sort((a, b) => a - b).join(', ')}]`);
                });
            }

            // Sum bets
            if (betData.sumBets && Array.isArray(betData.sumBets) && betData.sumBets.length > 0) {
                betParts.push(`Sum Bets: [${betData.sumBets.join(', ')}]`);
            }

            if (betParts.length > 0) {
                parts.push(betParts.join(' | '));
            }
        });
    ...
    // Format chispazo result for display
    const formatChispazoResult = (result) => {
        if (!result || !Array.isArray(result)) return '-';
        // Result is array of digits, but chispazo result is 10 digits (5 two-digit numbers)
        if (result.length === 10) {
            const numbers = [];
            for (let i = 0; i < 5; i++) {
    ...
    const fetchBettingHistory = async (page = 1, pageSize = 50) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/admin/chispazo/betting-history`, {
                params,
                headers: {
            ...
            console.error('Error fetching chispazo betting history:', err);
        ...
        try {
            ...
            if (response.data && response.data.data) {
                ...
                const rows = data.map(item => [
                    item.planNumber || '',
                    item.username || '',
                    formatChispazoBet(item.bet),
                    item.betAmount || 0,
                    dayjs(item.createdAt).format('YYYY-MM-DD HH:mm:ss'),
                    formatChispazoResult(item.result),
                    item.winAmount || 0,
                ...
                link.setAttribute('download', `chispazo_betting_history_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}.csv`);
    ...
    const historyColumns = [
        ...
        },
        {
            ...
            render: (result) => {
                if (!result) return '-';
                const numbers = Array.isArray(formatChispazoResult(result)) 
                    ? formatChispazoResult(result) 
                    : formatChispazoResult(result).split(' ');
                return (
    ...
    return (
        <div className="p-4">
            <Title level={2}>Chispazo Betting History</Title>
            <Card>
...
export default ChispazoBettingHistory;
