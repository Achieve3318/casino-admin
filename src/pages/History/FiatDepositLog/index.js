...
export default function FiatDepositLog({ username = "" }) {
      ...
      getFilterColumns([
        ...
        },
        {
          ...
          render: (amount, record) =>
            ...
            })} ${sitemode === 'mx' ? 'MXN' : sitemode === 'taka' ? 'BDT' : sitemode === 'brazil' || sitemode === 'grupo25' ? 'BRL' : 'GHS'}`,
        },
  ...
  const fetchFiatCount = useCallback(() => {
    postUrl(
      sitemode,
      `/api/${sitemode === 'mx' ? 'toppay' : sitemode === 'taka' ? 'worldpay' : (sitemode === 'brazil' || sitemode === 'grupo25') ? 'winpay' : 'wepay'}/getCollectionHistoryCount`,
      {
  ...
  const fetchFiatData = useCallback(() => {
    ...
    postUrl(
      sitemode,
      `/api/${sitemode === 'mx' ? 'toppay' : sitemode === 'taka' ? 'worldPay' : (sitemode === 'brazil' || sitemode === 'grupo25') ? 'winpay' : 'wePay'}/getCollectionHistory`,
      {
        filters: fiatParams.filters,
        sorter: fiatParams.sorter,
