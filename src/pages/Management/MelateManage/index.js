...
const MelateManage = () => {
    ...
    // Draw days: Wednesday (3), Friday (5), Sunday (0)
    const getNextDrawDate = (date) => {
        const currentDay = date.day();
        const drawDays = [0, 3, 5]; // Sunday, Wednesday, Friday
        
        // If today is a draw day and before 21:00, use today
    ...
    const fetchRecentResults = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${API_URL}/admin/melate/results/last-days?days=7`, {
                headers: {
    ...
    const fetchResultsByDate = async (date) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/admin/melate/results/date?date=${dateStr}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
            ...
            if (response.data && response.data.data && response.data.data.length > 0) {
                ...
                // Format result for display: "0105123456" -> "01 05 12 34 56"
                const formatted = formatMelateResult(result.result);
    ...
    const handleInputChange = (value) => {
        // Remove spaces and allow only digits, max 14 digits (7 two-digit numbers)
    ...
    const handleSetResult = async () => {
        // Validate: must be 14 digits (7 two-digit numbers)
        if (!inputResult || inputResult.length !== 14) {
            warning('Please enter 14 digits (7 two-digit numbers from 01-56)');
            return;
        }
        ...
        // Validate each number is between 01-56
        const numbers = [];
        for (let i = 0; i < 7; i++) {
            const num = parseInt(inputResult.substring(i * 2, (i + 1) * 2), 10);
            if (num < 1 || num > 56) {
                warning(`Number ${num} at position ${i + 1} is invalid. All numbers must be between 01-56.`);
                return;
            }
            numbers.push(num);
        }

        // Check if selected date is a draw day
        const selectedDay = selectedDate.day();
        const drawDays = [0, 3, 5]; // Sunday, Wednesday, Friday
        
        if (!drawDays.includes(selectedDay)) {
            warning('Selected date is not a draw day. Melate draws are on Wednesday, Friday, and Sunday.');
            return;
        }

        try {
            setLoading(true);
            // Create draw time: selected date at 21:00
            ...
            await axios.post(`${API_URL}/admin/melate/results/set`,
                {
                    result: inputResult,
            ...
            success('Melate result set successfully and bets processed');
            setInputResult('');
            setInputIssueNumber('');
            fetchRecentResults();
        } catch (err) {
            console.error('Error setting melate result:', err);
    ...
    const handleDeleteResult = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`${API_URL}/admin/melate/results/delete/${id}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
            success('Melate result deleted successfully');
            fetchRecentResults();
        } catch (err) {
            console.error('Error deleting melate result:', err);
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '40%',
            render: (result) => {
                const numbers = formatMelateResult(result).split(' ');
                return (
                    <div className="flex justify-center gap-2">
                        {numbers.map((num, idx) => {
                            const isSeventh = idx === 6; // 7th number (index 6)
                            return (
                                <div
                                    key={idx}
    ...
    return (
        <div className="p-4">
            <Title level={2}>Melate Results Management</Title>

            {/* Set Result Section */}
            <Card title="Set Melate Result" className="mb-4">
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                    <Alert
                        message="Melate Draw Schedule"
                        description="Melate draws are on Wednesday, Friday, and Sunday at 21:00 (9 PM) Mexico time. You can only set results after the draw time has passed."
                        type="info"
                        showIcon
                    ...
                    <div className='flex gap-4 items-center'>
                        <div className='flex gap-1 items-center'>
                            ...
                            <DatePicker
                                ...
                                disabledDate={(current) => {
                                    // Only allow selecting draw days (Wed, Fri, Sun)
                                    const day = current.day();
                                    return !([0, 3, 5].includes(day));
                                }}
                            />
                        </div>
                        <Text type="secondary">
                        ...
                        {!canSetResult() && (
                            <Text type="danger">
                                {selectedDate.day() === 0 || selectedDate.day() === 3 || selectedDate.day() === 5
                    ...
                    <div>
                        ...
                        <div className="mt-1">
                            <Text type="warning" style={{ fontSize: '12px' }}>
                                Note: First 6 numbers are used for draw evaluation. 7th number is not used.
                            </Text>
                        </div>
                    </div>
                    ...
                    <Space.Compact style={{ width: '100%', maxWidth: '600px' }}>
                        <Input
                            maxLength={20} // 14 digits + 6 spaces
                            value={displayInput}
                    ...
                    {inputResult && inputResult.length === 14 && (
                        <div className="flex justify-center gap-2 mt-4">
                            {formatMelateResult(inputResult).split(' ').map((num, idx) => {
            ...
            <Card title="Recent Melate Results (Last 7 Days)">
                <Table
                    columns={columns}
